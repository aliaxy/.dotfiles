...
def OpenApplicationIfThereIsNoOpen(
    application: str, command: str, workspace: str = "-1"
):
    if workspace == "-1":
    ...
    for con in i3.get_tree():
        if con.window and con.parent.type != "dockarea":
            if con.workspace().name == workspace and con.window_class == str(
                application
            ):
                find_application = 1
                break
...
if __name__ == "__main__":
    if (len(sys.argv)) < 3:
        print("error : len(sys.argv) dose not meet the requirements")
